...
$(function() {
    ...
    console.log('Skills', _.sortBy(_.uniq(_.flatten(_.map(data, 'skills')))));
    console.log('Names sorted by friends', _.map(_.orderBy(data, 'friends.length'), 'name'));
    console.log('Friends names', _.uniq(_.map(_.flatten(_.map(data, 'friends')), 'name')));
});
